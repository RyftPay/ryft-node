...
- `src/types/<resource>/` — response models plus request models in `req.ts` (older resources use `<resource>Req.ts`). Optional API fields are typed `field?: T | null | undefined`.
...
4. `src/index.ts` — export the client class and `export *` its types (several older resources were never exported here, so consumers can only reach them through the `Ryft` instance — follow the conversions pattern, don't copy the gaps)
...
Test files must end in `.test.ts` — `test/api/applePay/applePay.ts` is misnamed and silently never runs.
